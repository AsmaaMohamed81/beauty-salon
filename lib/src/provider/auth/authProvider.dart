  ...
  login() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (userModel.success == 1) {
        ...
        _prefs.setString("userData", json.encode(userModel.toJson()));
        NetworkUtil.userModel = userModel;
        print("User Data" + _prefs.get("userData"));
  ...
  singUp() async {
    ...
    Response response = await _utils.post("Api/register",
        body: FormData.fromMap({
          ...
          "user_name": name,
          "user_email": email,
          "m_image":  image == null ? null : await MultipartFile.fromFile(image.path)
        }));
    userModel = UserModel.fromJson(response.data);
    if (response.statusCode == 200) {
      if (userModel.success == 1) {
  ...
  editUserData() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (response.data["success"] == 1) {
        ...
        _prefs.setString("userData", json.encode(userModel.toJson()));
        NetworkUtil.userModel = userModel;
        print("edit_profile Data" + _prefs.get("userData"));
