...
class CreateNewOrderScreen extends StatefulWidget {
  ...
  const CreateNewOrderScreen({Key key,this.fromMain,this.storesServices,this.storeId}) : super(key: key);
...
class _CreateNewOrderScreenState extends State<CreateNewOrderScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    print("jhgfdfghjk"+
        NetworkUtil.KuafirTypeSelected
      );
    return Scaffold(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                          ...
                          child: InkWell(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Icon(Icons.access_time,color:Color(0xff707070) ,)
                              ],
                            ),
                          ),
                        ),
                ...
                SimpleTextField(
                  fillColor: Colors.white,
                  hint: NetworkUtil.userModel.member == null ?  tr("phone number") :  NetworkUtil.userModel.member.userPhone,
                      ...
                      InkWell(
                        onTap: (){
                          showDialog(
                              context: context,
                              builder: (_) {
                                return  ShowServicesDialog(storesServices: widget.storesServices,);
                              });
                        },
                        child: Container(
                          width: MediaQuery.of(context).size.width*0.4,
                ...
                Padding(
                  padding: const EdgeInsets.only(left: 30,right: 30,top: 0,bottom: 100),
                  child: InkWell(
                    onTap: (){
                      Provider.of<BookingProvider>(context, listen: false).Notes = note.text;
                      Provider.of<BookingProvider>(context, listen: false).discountCode = discountCode.text;
                        ...
                        if(!_accept){
                          CustomAlert().toast(title:tr("You must agree to the terms and conditions"));
                          return;
                        }
                        if(snapshot.selectedTimeVall== null){
                          CustomAlert().toast(title:tr("The time of the visit must be determined"));
                          return;
                        }
                        if(snapshot.selectedDateVall== null){
                          CustomAlert().toast(title:tr("The date of the visit must be specified"));
                          return;
                        }
                        if(snapshot.serviceSelected.length == 0){
                          CustomAlert().toast(title:tr("Service must be selected"));
                          return;
                        }
  ...
  Future<void> _selectTime(BuildContext context) async {
    ...
    if (picked_s != selectedTime )
      setState(() {
        ...
        selectedTimeVall =  "${selectedTime.hour}:${selectedTime.minute} ${selectedTime.period.name}";
        Provider.of<BookingProvider>(context, listen: false).selectedTimeVall = selectedTimeVall;
      });
  }
  _selectDate(BuildContext context) async {
    ...
    if (selected != selectedDate)
      setState(() {
        ...
        selectedDateVall = "${selectedDate.day}/${selectedDate.month}/${selectedDate.year}";
        Provider.of<BookingProvider>(context, listen: false).selectedDateVall = selectedDateVall;
