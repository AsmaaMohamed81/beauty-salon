...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({Key? key, @required this.StoreName, this.StoreId})
      : super(key: key);
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    try {
      socket = io(
          "http://nossl.alatheer.site:3000?user_id=7&username=r",
          <String, dynamic>{
                      ...
                      child: FloatingActionButton(
                        ...
                        onPressed: () async {
                          if (_messageController.text.trim().isNotEmpty) {
                            ...
                            Provider.of<ChateProvider>(context, listen: false)
                                .sendMessage(
                                    NetworkUtil!.userModel!.member.userId,
                                    widget.StoreId,
                                    message);
                            setState(() {
                              _messages.add(ChatModel(
                                  id: socket.id, to: "3", message: message));
                            });
